...
- [!] Known, deliberately-not-yet-resolved gaps: no `docs/seed/brand.md` (visual identity) or `docs/seed/market.md` (real market research) sibling exists; `API-005`/`API-006` have no dedicated `TC-###` (both cite `TC-004`); Facebook Graph API `debug_token` app-id verification (`security-compliance.md` T-010) is specified as mandatory but not yet confirmed as implemented (no code exists yet).
- [x] `UserLocation.radius_km` vs `Alert.radius_km` — RESOLVED 2026-09-18: `technical-design.md` Algorithm 2 confirms `UserLocation.radius_km` is the actual fanout query parameter (previously only proposed); fixed alongside a real F-004 bug found in the same algorithm (`decision-ledger.md`).
