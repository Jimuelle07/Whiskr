...
| TASK-010 | PH-03 | ingest-time cross-page/group dedup match, w/ graceful BR-008-failure handling (`decision-ledger.md` 2026-09-18 fix — a rejected scraped post must log+skip, never crash the ingest job); F-001 | TASK-004, TASK-005 | executor | `backend/src/ingestion/dedup.ts, backend/src/ingestion/__tests__/dedup.test.ts` | `npm --prefix backend test -- ingestion/dedup` | TC-001 pending | — | blocked | `npm --prefix backend test -- ingestion/dedup` |
