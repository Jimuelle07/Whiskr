...
| S17 | No-secret-in-response guard | *(not yet automated — [assumption]; propose a response-shape test asserting `PushToken.token`, `UserLocation.lat/lng`, and `Session.token_hash` never appear in any API JSON body — the raw session token is the sole exception, returned once at API-007, per `security-compliance.md`'s pre-milestone checklist)* | `PushToken.token`/`UserLocation.lat/lng`/`Session.token_hash` leaking into a client-facing response | test runner output, once written | executor (`TASK-003`/`TASK-012`/`TASK-013`), keeper — flagged as a gap to close at scaffold, not invented as an existing test here |
